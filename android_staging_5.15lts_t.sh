...
pushd Android
  /home/jenkins/bin/repo.google init -u https://github.com/intel-innersource/os.android.bsp-gordon-peak.manifests -b android/t/bxtp_ivi/master -m r1
  ...
  /home/jenkins/bin/repo.google manifest -r -o 5.15_android_t_manifest_original.xml #save original manifest
  ...
  if [[ $baseline == "False" ]]; then
    # Fetch kernel source code and config
    cd kernel
      rm -fr lts2018
  ...
  /home/jenkins/bin/repo.google manifest -r -o 5.15_android_t_manifest.xml #save manifest

  #workaround start:
    #fix lz4 issues
    #cp -f ./device/intel/mixins/groups/kernel/AndroidBoard.mk ./device/intel/mixins/groups/kernel/AndroidBoard_bak.mk
    #cp -f /home/jenkins/AndroidBoard.mk ./device/intel/mixins/groups/kernel/AndroidBoard.mk
  #end of workaround
