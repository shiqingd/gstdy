...
declare staging_number=${STAGING_NUMBER}
declare kernel_version=${BASELINE}
declare tag=""

if [[ $KERNEL == *rt* ]];then
    tag=svl-pre-si-linux-${kernel_version}-${STAGING_NUMBER}
    kernel_branch="svl/pre-si/linux-rt/${kernel_version}"
    overlay_branch="svl/pre-si/linux-rt/${kernel_version}-centos"
    overlay_tag=svl-pre-si-overlay-${kernel_version}-centos-${STAGING_NUMBER}
else
    tag=svl-pre-si-linux-${kernel_version}-${STAGING_NUMBER}
    kernel_branch="svl/pre-si/linux/${kernel_version}"
    overlay_branch="svl/pre-si/linux/${kernel_version}-centos"
    overlay_tag=svl-overlay-${kernel_version}-centos-${STAGING_NUMBER}
fi

pushd kernel-staging
    git remote add svl https://github.com/intel-innersource/os.linux.kernel.iot-next.git
    git reset --hard $tag
    git push origin HEAD:${kernel_branch}
    push_remote svl HEAD $kernel_branch
    git tag -d $tag || :
    tag $tag "IKT svl kernel yocto $kernel_version $STAGING_NUMBER"
    push_tag svl $tag
...
echo "KERNEL_TAG=${tag}" > svl_release.prop
echo "KERNEL=${KERNEL}" >> svl_release.prop
echo "STAGING_NUMBER=${STAGING_NUMBER}" >> svl_release.prop
...
<body class="c7">
  ...
  <br>
  <p class="c0"><span class="c2"><b>svl<b></span><span class= "c2"> release for $kernel_version is published.</span></p>
  <br>
