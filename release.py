...
def __main(args):
	...
	releases = KernelRelease.objects.filter(kernel=kobj)
	print("==============================================================================================")
		...
		with pushd(os.path.join(os.environ["WORKSPACE"],_set.repo.project), _verbose=True):
			...
			for row in releases:
				...
				c = Context ( { "is_sandbox" : '' , "is_cve" : str(_set.repo.repotype) == 'cve' and 'cve-' or '' ,
					"kernel" : kobj, "release" : row.release, "staging_number" : args.staging_number} )
				Tag = t.render (c)

				try:
					__traceable_git("checkout" , Tag)
				except Exception as e:
					print("unable to checkout", Tag, str(e).splitlines()[-1], ", SKIP release", row.release.name)
					continue
				if not all_remotes:
				...
				for remote in all_remotes:
					...
					if remote.kernelrelease.release == row.release:
						push_branch_template = remote.push_template.template.template
						t = Template(push_branch_template)
						c = Context( { "kernel" : kobj, "release" : row.release, "staging_number" : args.staging_number} )
						...
						if ( not remote.remote_repo.external ) and args.internal_release == 'True' :
						...
						if ( remote.remote_repo.external ) and args.external_release == 'True':
							if 'cve' not in remote.remote_name:
								...
								t = Template(Tag_template)
								c = Context({"is_sandbox": '', "is_cve": 'cve-', "kernel": kobj, "release": row.release,
								...
								with pushd(remote_repo_path, _verbose=True):
									try:
										...
										local_patch_path =  Template(bt.template.template).render(Context({ "repo_path" : local_repo_path , "release" : row.release }))
										remote_patch_path = Template(bt.template.template).render(Context({ "repo_path" : remote_repo_path , "release" : row.release }))
...
if __name__ == '__main__':
	...
	parser.add_argument('--external_release', '-e', required =True, type=str, help="Push the staging branches to external github branch")
	parser.add_argument('--internal_release', '-i', required =True, type=str, help="Push the staging branches to internal release branch")
