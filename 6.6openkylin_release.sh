...
#this script is only for 6.6 openkylin release
#usage:
# ./6.6openkylin_release.sh v6.6.16 241220T073318Z PKT-18856
...
STAGING_REV=lts-${BASELINE}-kylin-dev-${STAGING_NUMBER}
...
cd kernel-lts-staging
    #git checkout 6.6/kylin/dev/linux
    ...
    git push --dry-run kernel_lts HEAD:refs/heads/6.6/kylin/dev/linux
    git push --dry-run kernel_lts ${STAGING_REV}

    if [[ "$DRY_RUN" == False ]];then
        git push kernel_lts HEAD:refs/heads/6.6/kylin/dev/linux
...
Subject="[Release][$BASELINE][LTS-2023-openKylin]$STAGING_NUMBER"

cat message_6.6openkylin.html  | sed "s/\$BASELINE/$BASELINE/g" | sed "s/\$STAGING_NUMBER/$STAGING_NUMBER/g" | sed "s/\$TRACKING_JIRA/$TRACKING_JIRA/g" > ${Message}
