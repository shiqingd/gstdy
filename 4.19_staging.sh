...
quiltify() {
	...
	pushd $mydir/kernel-lts-staging
		git format-patch origin/4.19/release/$android --suffix=.$datetime
		if [ ! -e *${datetime}* ]; then
...
if [[ $kernel == "ANDROID_T" ]];then
    android=android_t
    config=config_t
elif [[ $kernel == "ANDROID_S" ]];then
    android=android_s
    config=config_s
fi
...
if $OPT_staging; then
	...
	if [ "${alternative_android_branch}" == "none" ] ; then
...
pushd kernel-lts-staging
  ...
  if [[ $STAGING_NUMBER != "none" ]]; then
    ...
    if [[ $? != 0 ]]; then
        ...
        echo "the tag doesn't exist in kernel-lts-staging repo, sandbox-staging-lts-${upstream_tag}-${android}-${datetime}, please check."
        exit 1
    else
...
if [ "$SANDBOX_TAG" == "sandbox" ] ; then
	tag_label_prop=$SANDBOX_TAG-lts-$upstream_tag-${android}-$datetime
	tag_label_cve_prop=$SANDBOX_TAG-lts-$upstream_tag-${android}-cve-$datetime
else
	tag_label_prop=lts-$upstream_tag-$android-$datetime
	tag_label_cve_prop=lts-$upstream_tag-$android-cve-$datetime
fi
tag_label=lts-$upstream_tag-$android-$datetime
tag_label_cve=lts-$upstream_tag-$android-cve-$datetime
...
pushd kernel-lts-staging
  ...
  update_config_timestamp x86_64 $working_dir/kernel-config/bxt/android/non-embargoed/x86_64_defconfig $android
popd


#1. go to 4.19/android_s or 4.19/android_t
#2. get get diff patches from last rc to HEAD in 4.19/android_s or android_t(quiltify)
...
pushd kernel-lts-staging
	ln -s $mydir/kernel-lts-cve/$android/patches .
	git quiltimport
	tag_sandbox $tag_label_cve $SANDBOX_TAG
	update_config_timestamp x86_64 $working_dir/kernel-config/bxt/android/non-embargoed/x86_64_defconfig $android
popd
...
if [[ $kernel == "ANDROID_T" ]];then
    subject_msg="[Staging][Android-T][$upstream_tag][LTS] $release_string"

    jobs="4.19-bkc-staging-android_t-build-test
    4.19-bkc-staging-gordon_peak_t-baseline
    4.19-bkc-staging-gordon_peak_t"
elif [[ $kernel == "ANDROID_S" ]];then
    subject_msg="[Staging][Android-S][$upstream_tag][LTS] $release_string"

    jobs="4.19-bkc-staging-android_s-build-test
...
for j in ${jobs}; do
    ...
    if [[ "$j" == "4.19-bkc-staging-gordon_peak_s" || "$j" == "4.19-bkc-staging-gordon_peak_t" ]];then
