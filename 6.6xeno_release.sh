...
#this script is only for 6.6xenomai release
#usage:
#  ./6.6xeno_release.sh 6.6xeno v6.6.15 240306T021358Z PKT-10899
...
cd kernel-lts-staging
  ...
  git push --dry-run kernel_lts HEAD:refs/heads/6.6/dovetail-xenomai
  git push --dry-run kernel_lts ${STAGING_REV}

  if [[ "$DRY_RUN" == False ]];then
    git push kernel_lts HEAD:refs/heads/6.6/dovetail-xenomai
  ...
  if [[ "$EXTERNAL" == True ]];then
    ...
    git push --dry-run linux_intel_lts HEAD:refs/heads/6.6/dovetail-xenomai
    git push --dry-run linux_intel_lts ${STAGING_REV}

    if [[ "$DRY_RUN" == False ]];then
      git push linux_intel_lts HEAD:refs/heads/6.6/dovetail-xenomai
...
cd kernel-lts-cve
  git reset --hard ${STAGING_REV_CVE}
cd -

cd linux-intel-lts
  #git checkout -B 6.6/dovetail-xenomai-cve
  git checkout 6.6/dovetail-xenomai-cve
  ...
  git push --dry-run origin 6.6/dovetail-xenomai-cve
  git push --dry-run origin ${STAGING_REV_CVE}
  git push origin 6.6/dovetail-xenomai-cve
...
Subject="[Release][$BASELINE][LTS-2023-STABLE-XENOMAI]$STAGING_NUMBER"

cat message_6.6xeno.html  | sed "s/\$BASELINE/$BASELINE/g" | sed "s/\$STAGING_NUMBER/$STAGING_NUMBER/g" | sed "s/\$TRACKING_JIRA/$TRACKING_JIRA/g" > ${Message}
