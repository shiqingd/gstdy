...
Usage()
{
    ...
    echo "example for lts: $0 v6.6.1 231128T083048Z  PKT-10289 "
    echo "example for rt : $0 v6.6.1-rt1 231128T083048Z PKT-10289 "
}
...
if [[ "${BASELINE}" == *rt* ]];then
    if [ ! -f message_6.6rt.html ];then
        echo "RT release email template message_6.6rt.html does not exist, please check"
        exit 1
    fi

    Subject="[Release][$BASELINE][LTS-2023-STABLE-RT] $STAGING_NUMBER"

    cat message_6.6rt.html  | sed "s/\$BASELINE/$BASELINE/g" | sed "s/\$STAGING_NUMBER/$STAGING_NUMBER/g" | sed "s/\$TRACKING_JIRA/$TRACKING_JIRA/g" > ${Message}

else
    if [ ! -f message_6.6lts.html ];then
        echo "LTS release email template message_6.6lts.html does not exist, please check"
        exit 1
    fi

    Subject="[Release][$BASELINE][LTS-2023] $STAGING_NUMBER"
    cat message_6.6lts.html  | sed "s/\$BASELINE/$BASELINE/g" | sed "s/\$STAGING_NUMBER/$STAGING_NUMBER/g" | sed "s/\$TRACKING_JIRA/$TRACKING_JIRA/g" > ${Message}
fi
