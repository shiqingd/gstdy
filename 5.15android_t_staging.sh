#!/bin/bash -ex
#this script is only for 5.15 android_t staing job
...
STAGING_REV=lts-${BASELINE}-android_t-${datestring}
STAGING_REV_CVE=lts-${BASELINE}-android_t-cve-${datestring}
...
cd kernel-lts-staging
  ...
  if [[ $ANDROID_STAGING_BRANCH == none ]];then
    echo "git checkout 5.15/android_t"
  ...
  if [[ $OPT_cve == "false" ]]; then
    ...
    git quiltimport --series=../kernel-lts-cve/android_t/patches/series --patches=../kernel-lts-cve/android_t/patches/
...
cd kernel-config
  ...
  if [[ $CONFIG_STAGING_BRANCH == none ]];then
    echo "git checkout 5.15/config_t"
  ...
  if [[ $OPT_cve == "false" ]]; then
    ...
    echo "cve_tag_name is $STAGING_REV_CVE"
    git tag $STAGING_REV_CVE -m ""
    git push origin $STAGING_REV_CVE
  fi
...
#generate prop file for downstream job, i.e., 5.15 android_t build job
rm -rf *.prop
if [[ $OPT_cve == "false" ]]; then
  echo "STAGING_REV=$STAGING_REV" > 5.15android_t.prop
  echo "KERNEL=5.15lts" >> 5.15android_t.prop
  echo "STAGING_REV=$STAGING_REV" > message.txt
else
  echo "STAGING_REV=$STAGING_REV_CVE" > 5.15android_t_cve.prop
  echo "KERNEL=5.15lts" >> 5.15android_t_cve.prop
  echo "STAGING_REV_CVE=$STAGING_REV_CVE" > message.txt
fi


echo "[Staging][${BASELINE}][Android_T]${datestring}" > subject.txt
#echo "STAGING_REV=$STAGING_REV" > message.txt
#echo "STAGING_REV_CVE=$STAGING_REV_CVE" >> message.txt
